...
$("#busqueda").click(e => {
  ...
  textoBuscado = (textoBuscado.trim()).replace(" ", "%20");

  if (textoBuscado === "") {
    return false;
  }
  else {
    let endpoint = `https://api.themoviedb.org/3/search/movie?api_key=105eb79aa1e6df60a2b95878ad2289aa&language=es&query=${textoBuscado}`;
...
const filtrarPeliculas = ({ results }) => {
  ...
  results.forEach((peli) => {
    ...
    if (peli.poster_path === null) {
      imagen.css("background-image", `url("imagen_null.jpg")`);
    }
    else {
      imagen.css("background-image", `url("https://image.tmdb.org/t/p/w500${peli.poster_path}")`);
    }
    ...
    let cardTextRelease = $(`<p class="card-text text-right"></p>`);
    let smallTextRelease = $(`<small class="text-muted">Fecha de Lanzamiento: ${peli.release_date}</small>`);

    cardTextRelease.append(smallTextRelease);
    ...
    let cardFooter = $(`<div class="card-footer text-muted text-center"></div>`);
    let cardTextFooter = $(`<p class="card-text text-center"></p>`);
    let iTextFooter = $(`<i class="fas fa-star"></i> ${peli.popularity}
                         <i class="fa fa-thumbs-up"></i> ${peli.vote_average}
                         <i class="fa fa-thumbs-up"></i> ${peli.vote_count} `);

    cardTextFooter.append(iTextFooter);


    cardHeader.append(cardTitle);
    cardBody.append(cardTextRelease);
    cardBody.append(cardText);
    cardFooter.append(cardTextFooter);
